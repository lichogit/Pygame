...
# Asset filenames (put these in the same folder as the script)
...
def slice_sprite_sheet(surf):
    """If sprite is a horizontal strip of square frames, slice into frames.
       Return list of frames (at least one)."""
    w, h = surf.get_size()
    if w >= h:
        frame_w = h
        frames = []
        for x in range(0, w, frame_w):
            rect = pygame.Rect(x, 0, frame_w, frame_w)
            if rect.right <= w:
